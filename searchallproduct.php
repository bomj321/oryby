...
//											$sql="SELECT * FROM `categories`  ";
 ...
 $query=$_GET['keyword'];
             if($title !=" ")
			 {
 			 $query="SELECT * FROM products INNER JOIN categories ON(products.catid=categories.catid)WHERE (products.ntitle LIKE '%".$title."%' )";
				}	 
			if($keyword !="")
			 {
 			 $query="SELECT * FROM products INNER JOIN categories ON(products.catid=categories.catid)WHERE (products.ntitle LIKE '%".$query."%' )";
				}			
