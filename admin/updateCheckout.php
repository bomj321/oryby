...
$row=mysqli_fetch_assoc($stmt);
     $title = $row['title'];// item name
		      ...
		      $description = $row['descripcion'];// item name
			    $tipo = $row['tipo1'];// item name
...
if(isset($_POST['btn_save_updates']))
	{
     ...
     // PARA COMPROBAR SI HAY TIPO DE ELEMENTOS
     if ($tipo!= ' ' ) {
       $description = $_POST['description'];// item name


       $sql="UPDATE checkout  SET  descripcion='".$description."'  WHERE (id='$id')";
       ...
       trigger_error('Wrong SQL: ' . $sql . ' Error: ' . $connection->error, E_USER_ERROR);
           ...
           window.location.href="manageCheckout.php";
               </script>
                       <?php
             }
             else{
