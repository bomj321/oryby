...
echo $uzerid=$r1['user_id'];


echo $stquery2="SELECT * FROM users where user_id='$uzerid'";
...
?>
    ...
    <body>
                        ...
                        <div class="row">
                            ...
                            <form action="mycartArry.php?pid=<?php echo $pid; ?>" method="post" >        
                               ...
                               <hr class="spacer-15">
                                ...
                                <ul class="list list-inline">
     <li><i class="fa fa-shopping-basket mr-5"><input type="submit"  value="Buy Now" class="btn btn-default btn-md round"></i>
	 <button type="submit"  class="btn btn-default btn-md round"><i class="fa fa-shopping-basket mr-5"></i>Add to cart</button</li>
	 </form>
     <li><button type="button" class="btn btn-gray btn-md round" data-toggle="modal" data-target="#myModal"><i class="fa fa-heart mr-5"></i>Ask Supplier</button></li>
      <li><button type="button" class="btn btn-success btn-md round" data-toggle="modal" data-target="#myModal"><i class="fa fa-address-book-o"></i>Contact Supplier</button></li>
 ...
 <div class="modal fade" id="myModal" role="dialog">
  ...
  <!-- END Modal content-->           
			...
			<?php 
			$userid=$row['user_id'];
			$qrty="SELECT * FROM users where user_id=$userid";
			$rzlt=mysqli_query($connection,$qrty);
			$rwd=mysqli_fetch_array($rzlt);
			 $suppliermail=$rwd['email'];
			
			?>								
                                            <dt>Siupplier Name</dt>
                                            <dd><?php echo $rwd['firstName'] .' '.$rwd['lastName']; ?></dd>
                                            <dt>Size</dt>
                                            <dd>any size</dd>
                                        </dl>
                                    </div><!-- end col -->
			...
			$qrty="SELECT * FROM seller Where  email='$suppliermail'";
			$rslt=mysqli_query($connection,$qrty);
			//echo $nr= mysqli_num_rows($rslt);
			$rd=mysqli_fetch_array($rslt);
			
			
			?>
                                        <dl class="dl-horizontal">
                                            <dt>Company </dt>
                                            <dd><?php echo $rd['company_name'];?></dd>
                                            <dt>Country</dt>
                                            <dd><?php echo $rd['countryName'];?></dd>
                                        </dl>
                                    </div><!-- end col -->
                                </div><!-- end row -->
                            </div><!-- end tab-pane -->
			...
			<?php	 if(isset($_POST['buyerquery'])){
$msg=$_POST['qustion'];
$_POST['email'];
$headers = "MIME-Version: 1.0" . "\r\n";
//$headers .= "Content-type:text/html;charset=UTF-8" . "\r\n";
$to=$suppliermail;
//$to="anwarattiq@gmail.com";
...
if($result){
	//$smsg = '<div class="alert alert-success col-md-4" id="suc">Successfuly Sent !</div>';
    //echo $smsg;
?>
<script> alert("Successfuly sent !") </script>
