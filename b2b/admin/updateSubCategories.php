...
include('Connect.php');
 ...
 $sql="SELECT * FROM subcategories INNER JOIN categories ON(subcategories.catid = categories.catid)  Where subcatid='$subcatid'";
...
$sql="UPDATE subcategories  SET  catid='".$catid."', subtitle='".$subtitle."', subcategoryimage='".$image."'  WHERE (subcatid='$subcatid')";
...
trigger_error('Wrong SQL: ' . $sql . ' Error: ' . $connection->error, E_USER_ERROR);
			...
			if($stmt->execute())
			{
				...
				?>
                <script>
				        alert("Sub Category Updated!");  //not showing an alert box.
