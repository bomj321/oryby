...
if(isset($_SESSION["cart"])) //Post Data received from product list page.
{
	...
	$i = 0;
        ...
        {	
        $product_code 	= filter_var($cart_itm["pid"], FILTER_SANITIZE_STRING); 
	
		 $results = "SELECT * from products where pid=$product_code";
		
		$sql=mysqli_query($connection,$results);
		$row=mysqli_fetch_array($sql);
		
        $paypal_data .= '&L_PAYMENTREQUEST_0_NAME'.$i.'='.urlencode($row[ntitle]);
        $paypal_data .= '&L_PAYMENTREQUEST_0_NUMBER'.$i.'='.urlencode($cart_itm["p_id"]);
        $paypal_data .= '&L_PAYMENTREQUEST_0_AMT'.$i.'='.urlencode($row[price]);		
      ...
      $subtotal = $_SESSION['toatl'];
		...
		$paypal_product['items'][] = array('itm_name'=>$obj->p_title,
											'itm_price'=>$obj->p_price,
											'itm_code'=>$cart_itm["p_id"], 
											);
		$i++;
    }
	...
	$padata = 	'&METHOD=SetExpressCheckout'.
		...
		{
			//	unset($_SESSION["cart"]); //session no longer needed
